...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const rt of resourceTypes) {
      await prisma.resourceType.upsert({ where: { slug: rt.slug }, update: {}, create: rt });
    }

    const bba = await prisma.program.upsert({
      where: { slug: "bba" },
      update: {},
      create: { name: "BBA", slug: "bba", description: "Bachelor of Business Administration" },
    });

    const thp = await prisma.program.upsert({
      where: { slug: "thp" },
      update: {},
      create: { name: "THP", slug: "thp", description: "Talent Hunt Program" },
    });

    type CourseEntry = { name: string; code: string };
    ...
    const bbaCourses: Record<number, CourseEntry[]> = {
      ...
      4: [
        ...
        { name: "Organisational Behaviour", code: "MGT-402" },
    ...
    for (let semNum = 1; semNum <= 8; semNum++) {
      const semester = await prisma.semester.upsert({
        ...
        update: {},
        create: { number: semNum, label: `Semester ${semNum}`, programId: bba.id },
      });
      for (const course of bbaCourses[semNum] ?? []) {
        await prisma.course.upsert({
          where: { semesterId_code: { semesterId: semester.id, code: course.code } },
          update: { name: course.name },
          create: { name: course.name, code: course.code, semesterId: semester.id },
        });
      }
    }

    const thpCourses: Record<number, CourseEntry[]> = {
      1: [
    ...
    for (let semNum = 1; semNum <= 4; semNum++) {
      const semester = await prisma.semester.upsert({
        ...
        update: {},
        create: { number: semNum, label: `Semester ${semNum}`, programId: thp.id },
      });
      for (const course of thpCourses[semNum] ?? []) {
        await prisma.course.upsert({
          where: { semesterId_code: { semesterId: semester.id, code: course.code } },
          update: { name: course.name },
          create: { name: course.name, code: course.code, semesterId: semester.id },
    ...
    const hashedPassword = await bcrypt.hash("Admin@SIBA2024", 12);
    await prisma.adminUser.upsert({
      where: { email: "admin@siba.edu.pk" },
      update: {},
      create: { email: "admin@siba.edu.pk", name: "SIBA Admin", password: hashedPassword, role: "SUPER_ADMIN" },
    });
